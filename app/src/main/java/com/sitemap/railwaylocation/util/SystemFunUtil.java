...
public class SystemFunUtil {
    ...
    public static final int SYSTEM_IMAGE = 0;//拍摄图片
    ...
    public static final int SYSTEM_IMAGE_PREPOSITION = 4;//拍摄图片 前置摄像头
        ...
        try {
            switch (type) {
                ...
                case SYSTEM_IMAGE_PHONE://本地相册
                    ...
                    break;
                case SYSTEM_IMAGE_PREPOSITION://拍摄图片(前置摄像头)
                    Intent img1 = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
                    FileNames name1 = new FileNames();
                    File appFile1 = createRootDirectory(APP_FILE_NAMES);
                    File file1 = new File(appFile1.getPath(),name1.getImageName());
                    this.setImgFile(file1);
                    img1.putExtra("camerasensortype", 2); // 调用前置摄像头
//                    img1.putExtra("android.intent.extras.CAMERA_FACING", 1);
                    img1.putExtra("autofocus", true); // 自动对焦
                    img1.putExtra("fullScreen", false); // 全屏
                    img1.putExtra("showActionIcons", false);
                    img1.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(file1));
                    context.startActivityForResult(img1, flag);
                    break;
                default:
                    break;
            }
